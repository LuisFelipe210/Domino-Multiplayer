...
export const joinRoom = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!roomExists) {
            ...
            const roomDetailsToSet: { [key: string]: string } = { 
                playerCount: '0', 
                status: 'waiting' 
            };

            if (password) {
                roomDetailsToSet.passwordHash = await bcrypt.hash(password, 10);
            }
            
            await redisClient.hSet(`room:${roomName}`, roomDetailsToSet);
